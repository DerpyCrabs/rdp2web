...
/// Encoded AVC420 bitmap data for a surface region.
#[derive(Debug)]
pub struct EncodedAvc420Frame<'a> {
    pub surface_id: u16,
    pub destination_rectangle: ExclusiveRectangle,
...
pub trait GraphicsPipelineHandler: Send {
    ...
    fn on_avc420_frame(&mut self, _frame: &EncodedAvc420Frame<'_>) {}
...
impl GraphicsPipelineClient {
    ...
    fn decode_avc420(&mut self, surface_id: u16, dest_rect: &ExclusiveRectangle, bitmap_data: &[u8]) -> PduResult<()> {
        ...
        self.handler.on_avc420_frame(&EncodedAvc420Frame {
            surface_id,
        ...
        let Some(ref mut decoder) = self.h264_decoder else {
            trace!("No H.264 decoder configured, forwarded encoded AVC420 frame only");
            return Ok(());
        };
